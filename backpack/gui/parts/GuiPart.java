...
public abstract class GuiPart {
    ...
    public void drawForegroundLayer(FontRenderer fontRenderer, int x, int y) {
        String text = inventory.isInvNameLocalized() ? inventory.getInvName() : StatCollector.translateToLocal(inventory.getInvName());
        int xOffset;
        switch(textPosition) {
            case LEFT:
                xOffset = 8;
    ...
    public void drawBackgroundLayer(float f, int x, int y) {
        ...
        Minecraft.getMinecraft().renderEngine.bindTexture(Constants.guiCombined);
